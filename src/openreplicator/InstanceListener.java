    ...
    public void onEvents(BinlogEventV4 binogEvent) {
        ...
        switch(eventType){
            ...
            case MySQLConstants.TABLE_MAP_EVENT://每次ROW_EVENT前都伴随一个TABLE_MAP_EVENT事件，保存一些表信息，如tableId, tableName, databaseName, 而ROW_EVENT只有tableId
            {
                ...
                logger.trace("TABLE_MAP_EVENT:tableId:{}",tableMapEvent.getTableId());
                break;
            }
            case MySQLConstants.DELETE_ROWS_EVENT:
            {
                ...
                for(Row row:rows){
                    ...
                    if(beforeMap !=null && beforeMap.size()>0){
                        CDCEvent cdcEvent = new CDCEvent(deleteEvent,databaseName,tableName);
            ...
            case MySQLConstants.UPDATE_ROWS_EVENT:
            {
                ...
                for(Pair<Row> p:rows){
                    ...
                    if(beforeMap!=null && afterMap!=null && beforeMap.size()>0 && afterMap.size()>0){
                        CDCEvent cdcEvent = new CDCEvent(updateEvent,databaseName,tableName);
                        //cdcEvent.setIsDdl(false);
                        //cdcEvent.setSql(null);
                        cdcEvent.setBefore(beforeMap);
            ...
            case MySQLConstants.WRITE_ROWS_EVENT:
            {
                ...
                for(Row row: rows){
                    ...
                    if(afterMap!=null && afterMap.size()>0){
                        CDCEvent cdcEvent = new CDCEvent(writeEvent,databaseName,tableName);
                        //cdcEvent.setIsDdl(false);
                        //cdcEvent.setSql(null);
                        cdcEvent.setAfter(afterMap);
                        CDCEventManager.queue.addLast(cdcEvent);
            ...
            case MySQLConstants.QUERY_EVENT:
            {
                ...
                CDCEvent cdcEvent = new CDCEvent(queryEvent,databaseName,tableName);
